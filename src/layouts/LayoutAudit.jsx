...
import { getTemplateAndProduct, getStockLastUpdate } from "../api/users/home";
...
/* ================================
 * format เวลาไทย (พ.ศ.) + Asia/Bangkok
 * output: 23/12/2568 13:45
 * ================================ */
const formatThaiBuddhistDateTime = (value) => {
  if (!value) return "-";
...
const LayoutAudit = () => {
  ...
  // ✅ โหลดเวลา stock ล่าสุด (ยิงครั้งเดียวตอน mount)
  useEffect(() => {
    let alive = true;
    ...
    const loadStockMeta = async () => {
      ...
      try {
        const res = await getStockLastUpdate();
        if (alive) setStockMeta(res || { updatedAt: null, rowCount: 0 });
      } catch (e) {
        console.error("StockLastUpdate load error:", e);
        if (alive) setStockMeta({ updatedAt: null, rowCount: 0 });
      } finally {
        if (alive) setStockMetaLoading(false);
      }
    };
  ...
  const handleAddProduct = async (item) => {
    ...
    try {
      const payload = { ...item, branchCode: item.branchCode || selectedBranchCode };
  ...
  /* =========================
   * Update shelf order (ลากเมาส์ แล้ว Save)
   * ========================= */
  const handleUpdateShelfProducts = async (serverItems, uiDraft) => {
    ...
    setData((state) => {
      ...
      const patch = new Map(
        (uiDraft || []).map((x) => [
          `${branch}|${shelf}|${Number(x.codeProduct)}|${Number(x.prevRowNo)}|${Number(x.prevIndex)}`,
      ...
      const next = state.map((p) => {
        ...
        const k = `${branch}|${shelf}|${Number(p.codeProduct)}|${Number(p.rowNo)}|${Number(p.index)}`;
  ...
  const displayedShelves = useMemo(() => {
    ...
    return groupedShelves
      .filter((shelf) => selectedShelves.length === 0 || selectedShelves.includes(shelf.shelfCode))
      .map((shelf) => {
        const matched =
          qRaw === ""
            ? shelf.shelfProducts
  ...
  }, [groupedShelves, selectedShelves, deferredSearch]);
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex flex-col leading-tight">
                ...
                <div className="flex items-center gap-2 text-[10px] sm:text-xs text-slate-500 whitespace-nowrap">
                  <span>ตรวจสอบและปรับปรุง POG</span>
                    ...
                    {stockMetaLoading ? (
                      ...
                      <>
                        ...
                        <span className="font-semibold text-slate-700">
                          {formatThaiBuddhistDateTime(stockMeta?.updatedAt)}
                        </span>
                      </>
                    )}
                  </span>
                ...
                <Suspense fallback={<div className="text-sm text-gray-500">Loading shelves...</div>}>
                  {displayedShelves.map((shelf) => (
                    <ShelfCardAudit
                      ...
                      onUpdateProducts={handleUpdateShelfProducts}
                      autoOpen={String(deferredSearch || "").trim().length > 0}
                    />
                  ))}
                </Suspense>
              </section>
